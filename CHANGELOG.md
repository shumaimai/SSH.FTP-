...
## [Unreleased]
### 追加
- **複数ウィンドウで接続**(Issue #14 段階1)。プロファイルの右クリック「新しいウィンドウで接続」
  または Ctrl+ダブルクリックで、独立したウィンドウに接続を開けるようにした。プロファイル /
  既知ホスト / 認証情報 / 設定は全ウィンドウで共有する。既存のタブ方式はそのまま維持
  (段階2「起動画面をサーバー選択にして常に新ウィンドウ」は #14 で方針確認中)。
